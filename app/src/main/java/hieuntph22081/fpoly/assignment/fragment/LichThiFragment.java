...
import hieuntph22081.fpoly.assignment.service.LichThiService;
...
public class LichThiFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView = view.findViewById(R.id.recyclerViewLichThi);
        lichHocDAO = MyDatabase.instance.lichHocDAO();
        Intent intent = new Intent(getContext(), LichThiService.class);
        getContext().startService(intent);
        filter = new IntentFilter("hieuntph22081.fpoly.assignment.LICH_THI");
        ...
        txtTimLop.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (txtTimLop.getText().toString().trim().length() == 0) {
                    ...
                    adapter = new LichHocAdapter(getContext(), lichHocDAO.searchLichThi("%" + txtTimLop.getText().toString().trim() + "%"));
    ...
    BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            lichHocs = (List<LichHoc>) intent.getExtras().getSerializable("lichThi");
